...
class SNGraphDataset(GraphDataset):
    ...
    def initialize(cls, distfn, opt, idx, objects, max_norm=1):
        conf = []
        print ('3', cls)
        model_name = cls.model_name % (opt.dset, opt.distfn, int(opt.dim))
        #model_name = cls.model_name ("'{}''{}''{}'".format((opt.dset), (opt.distfn), (opt.dim)))
#        print ('3')
        #model_name = cls.model_name ('%s', '%s', '%d'%(opt.dset, opt.distfn, opt.dim))
#        print ('4')
