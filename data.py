...
def parse_seperator(line, length, sep='\t'):
    d = line.strip(delimiter).split(sep)
    if len(d) == length:
        w = 1
    elif len(d) == length + 1:
        w = int(d[-1])
        ...
        raise RuntimeError ('Malformed input ({line.strip()})')
        return tuple(d) + (w,)
...
def iter_line(fname, fparse, length=2, comment='#'):
    fname = '/Users/sanika/Downloads/Data_Science_Seminar/facebook.txt'
...
def slurp(fin, fparse=parse_tsv, symmetrize=False):
    ...
    print ('slurp: objects={len(objects)}, edges={len(idx)}')
    return idx, objects